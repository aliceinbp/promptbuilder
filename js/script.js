...
                // JAVÍTÁS: Dupla görgetősáv eltávolítása
...
                    // Ezt a sort töröltük, mert feleslegesen hozzáadott egy második görgetősávot.
...
        // JAVÍTÁS: A "Hozzáad" gomb eseménykezelője
        document.querySelectorAll('.add-button').forEach(button => {
            button.addEventListener('click', function() {
                // Megkeressük a gomb szülő elemét (.prompt-section)
                const parentSection = this.closest('.prompt-section');
                if (!parentSection) return;

                // A szülőn belül megkeressük a select elemet és annak az ID-jából kinyerjük a kategóriát
                const selectElement = parentSection.querySelector('select');
                const category = selectElement.id.replace('-select', '');
                
                // A kategória alapján megszerezzük a megfelelő Choices.js példányt
                const choice = choiceInstances[category];
                const selectedValue = choice ? choice.getValue(true) : null;
                
                // Ha van kiválasztott érték...
                if (selectedValue && selectedValue !== "") {
                    // ...megszerezzük a cél szövegdobozt...
                    const targetTextarea = textareas[category];
                    // ...és hozzáadjuk az értéket.
                    targetTextarea.value += (targetTextarea.value.trim() !== "" ? ", " : "") + selectedValue;
                    updateFinalPrompt();
                    
                    // Végül töröljük a kiválasztást a legördülő menüből.
                    choice.clearInput();
...
    // Blog funkciók indítása, csak ha a megfelelő oldalon vagyunk
